...
namespace parametry
{
    ...
    {
        public global GL; tcp.tcp_client TCP = new tcp.tcp_client();Form1 glForm = new Form1();
        public int SaveSet = 0;
        public parametry()
        {
            InitializeComponent();
        }
        private void parametry_FormClosing(object sender, FormClosingEventArgs e)
        {
            //if (SaveSet == 0) { сохранить_параметры_Click(null,null); }
        }
        private void сохранить_параметры_Click(object sender, EventArgs e)
        {
            settings_copy(); 
            GL.IP = text_ip_new.Text;
            GL.MASKA = text_maska_new.Text;
            GL.DPORT = Convert.ToInt32(text_dport_new.Text); glForm.save_setting(GL); SaveSet = 3; MessageBox.Show("Параметры успешно сохранены!");
        }
        private void получить_параметры_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Вы уверены, что текущий IP-адрес соответствует подключенному\n ПТК КМ-Дельта?",
                "", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
            {
                GL.IP = text_ip_old.Text;
                GL.MASKA = text_maska_old.Text;
                GL.DPORT = Convert.ToInt32(text_dport_old.Text);
                glForm.poluchit_proshivku(ref GL); refreshS();
            }
            else { MessageBox.Show("Установите текущий IP-адрес в соответствии с подключенному\n ПТК КМ-Дельта"); }
        }
        private void settings_copy()
        {
            ...
            GL.otkl_panel = otkl_panel.Checked;
            GL.porog_min = (Int16)numeric_minimum.Value;
            GL.porog_max = (Int16)numeric_maximum.Value;
        }
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (f == 0)
            {
                ...
                glForm.save_setting(GL);
                this.Close(); return;
            }
            if (f == 1) { MessageBox.Show("Ошибка формата данных"); this.Close(); return; }
            if (f == 2) { MessageBox.Show("Нет связи!"); this.Close(); return; }
        }
        void refreshS() {
            try
            {
                ...
                otkl_panel.Checked = GL.otkl_panel;
                /* для задержек                   
                    0 мс
                    117 мс
                    468 мс
